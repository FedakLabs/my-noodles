...
import { loadEnvironmentManifest } from '../../scripts/load-environment.mjs';
...
export function createEnvironmentWranglerConfig(cloudflareDirectory, component, target) {
  ...
  const generated = source.replace(/("name"\s*:\s*)"[^"]+"/, `$1${JSON.stringify(target.workerName)}`);

  if (generated === source) {
    throw new Error(`Wrangler config does not contain a Worker name: ${sourcePath}`);
  }
