...
function BrandSkinsGallery() {
  ...
  return (
    <Stack spacing={3} sx={{ maxWidth: 960 }}>
      <SkinTierIntro
        tier="brand"
        description="Brand-level skins win over country when both are present. Same CSS variable contract as country skins."
      />
      <SkinRegistryTable
        title="Brand registry"
        description="Keys match Brand.themeKey / normalized brand slug. Add entries here before tagging products in the DB."
        rows={registryRows}
      />
      <SkinCardGrid>
...
function CategorySkinsGallery() {
  ...
  return (
    <Stack spacing={3} sx={{ maxWidth: 960 }}>
      <SkinTierIntro
        tier="category"
        description="Category skins apply when brand and country do not match. Useful for aisle-level discovery pages."
      />
      <SkinRegistryTable
        title="Category registry"
        description="Keys match Category.themeKey / normalized category slug."
        rows={registryRows}
      />
      <SkinCardGrid>
...
export const Brands: Story = {
  render: () => <BrandSkinsGallery />,
};

export const Categories: Story = {
  render: () => <CategorySkinsGallery />,
};
...
export const HashFallback: Story = {
  render: () => {
    const skin = resolveSkin({ slug: 'mystery-mochi' });
    return (
      <Stack spacing={1}>
        <Typography variant="overline" color="text.secondary">
          hash(slug) whisper fallback
        </Typography>
        <Box style={skinVarsToStyle(skin.cssVars)} sx={{ width: 168 }}>
          <Card
            sx={{
          ...
          >
            <CardContent>
              <Typography variant="subtitle2">mystery-mochi</Typography>
              <Typography variant="caption" color="text.secondary">
                hue {skin.definition?.bgHueBrand}° · source: {skin.source}
              </Typography>
              <Button size="small" variant="contained" sx={{ mt: 1 }}>
                Спробувати
              </Button>
...
export const ResolutionOrder: Story = {
  render: () => (
    <Stack spacing={2} sx={{ maxWidth: 720 }}>
      <Typography variant="body2" color="text.secondary">
        First registered match wins. Unregistered keys fall through silently.
      </Typography>
      {(
        [
